...
class Pool {
  constructor(factory) {
    ...
    factory.min = parseInt(factory.min);
    if (factory.min < 0 || factory.min !== Math.round(factory.min)) {
    ...
    factory.max = parseInt(factory.max);
    if (factory.max <= 0 || factory.max !== Math.round(factory.max)) {
