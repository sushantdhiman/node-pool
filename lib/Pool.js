...
class Pool {
  constructor(factory) {
    ...
    factory.min = Number(factory.min) > -1 ? Number(factory.min) : false;
    factory.max = Number(factory.max) > -1 ? Number(factory.max) : false;
  
    if (!factory.min) {
      throw new Error("factory.min must be >= 0");
    }
    
    if (!factory.max) {
      throw new Error("factory.max must be >= 0");
    }
